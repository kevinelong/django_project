...
def data_categories_dict(request):
    output = {}
    for c in Category.objects.all():
        output[c.id] = {"name": c.name}
    return HttpResponse(json.dumps(output, indent=4), content_type='application/json')


def data_categories_count(request):
    return HttpResponse('{"count":' + str(len(Category.objects.all())) + '}', content_type='application/json')
...
def data_both(request):
    page_list = []
    for p in Page.objects.all():
        page_list.append({"id": p.id, "title": p.title, "url": p.url, "category_id": p.category.id})

    category_list = {}

    for c in Category.objects.all():
        category_list[c.id] = {"name": c.name}

    return HttpResponse(json.dumps({"pages": page_list, "categories": category_list}, indent=4),
                        content_type='application/json')


def data_nested(request):
    output = []
    for c in Category.objects.all():
        category_dict = {"id": c.id, "name": c.name}
        pages_list = []
